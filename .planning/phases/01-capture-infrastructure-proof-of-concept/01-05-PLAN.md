...
type: execute
wave: 4
depends_on: ["01-03", "01-04A", "01-04B"]
...
must_haves:
  truths:
    - "Capture shows increasing packet counts"
    - "Application stable under high traffic"
    - "Traffic volume logged against 10MB/hour threshold"
    - "Capture stops gracefully when stopped"
    - "MTGO traffic visible in capture (optional verification)"
  artifacts:
...
<task type="auto">
  <name>Task 3: Add traffic volume logging for BPF effectiveness</name>
  <files>src-tauri/src/capture/loop.rs</files>
  <action>
Add traffic volume logging to capture loop in src-tauri/src/capture/loop.rs:

Add this logging logic in the capture_loop function, after the throughput calculation block:
...
This logging addresses Success Criterion #5 by:
1. Continuously tracking traffic volume in bytes/second
2. Comparing against the 10MB/hour threshold (≈2913 bytes/s)
3. Warning when threshold is exceeded (indicating filter is too broad)
4. Confirming when within threshold (indicating effective filtering)

The current broad filter ("outbound and tcp") will likely exceed the threshold during normal usage. This is expected in Phase 1. In Phase 2, we will refine the filter based on actual MTGO server IPs/ports discovered through capture analysis (see Task 3 in Plan 03).

DO NOT automatically stop capture or enforce threshold - this is informational logging only. The filter refinement happens in Phase 2 after analyzing captured traffic.
  </action>
  <verify>grep -E "(MAX_BYTES_PER_SECOND|Traffic volume.*exceeds|Traffic volume.*within)" src-tauri/src/capture/loop.rs</verify>
  <done>Traffic volume logging added with threshold comparison against 10MB/hour</done>
</task>

<task type="auto">
  <name>Task 4: Update main.rs with loop module</name>
...
2. **Run Application as Administrator:**
   ...
   - Verify UI shows "Admin: ✓ Yes" and "WinDivert Driver: ✓ Found"

3. **Test Basic Capture (any network traffic):**
...
5. **Test Error Handling:**
  ...
  <resume-signal>Type "approved" if basic capture works correctly, or describe issues with packet reception, WinDivert errors, or UI problems.</resume-signal>
</task>

<task type="checkpoint:human-verify" gate="non-blocking">
  <what-built>Optional verification with actual MTGO client traffic to validate filter effectiveness.</what-built>
  <how-to-verify>
**NOTE: This step is optional and does not block plan completion. If MTGO client is unavailable, skip this step and proceed to the next plan.**

1. **Test With MTGO Client:**
...
This validates that the broad "outbound and tcp" filter successfully captures MTGO traffic. In Phase 2, we will refine the filter based on actual MTGO server IPs/ports discovered through this testing.
  </how-to-verify>
  <resume-signal>Type "approved" if MTGO capture works, or "skipped" if MTGO client unavailable.</resume-signal>
</task>
