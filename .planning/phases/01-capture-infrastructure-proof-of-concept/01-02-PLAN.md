...
must_haves:
  truths:
    - "Application displays admin privilege status"
    - "Application displays WinDivert driver status"
    - "Errors displayed to user when capture fails"
    - "Clear messages guide users to restart as Admin or download WinDivert"
  artifacts:
